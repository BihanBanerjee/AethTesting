...
import type { CodeGenerationResult } from "../code-generation/types";
import type { UnifiedResponse, FileReference, Suggestion } from "../code-generation/response-types";
import { 
...
export class UnifiedResponseAdapter {
  ...
  private static determineContentType(result: CodeGenerationResult, intent: string): 'text' | 'code' | 'markdown' | 'json' {
    return determineContentType(result, intent);
  }
  ...
  /**
   * Filter warnings to only user-facing ones (uses shared utility)
   */
  private static filterUserFacingWarnings(warnings: string[]): string[] {
    return filterUserFacingWarnings(warnings);
  }

  /**
   * Extract insights from warnings (uses shared utility)
   */
  private static extractInsights(warnings: string[]): string[] {
    return extractInsights(warnings);
  }
